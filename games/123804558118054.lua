...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function()
			return game:HttpGet('https://raw.githubusercontent.com/7GrandDadPGN/Velocity/'..readfile('velo/profiles/commit.txt')..'/'..select(1, path:gsub('velo/', '')), true)
		end)
...
if isfile('velo/games/'..vape.Place..'.lua') then
	loadstring(readfile('velo/games/'..vape.Place..'.lua'), 'bedwars')()
else
